...
export default function ElementsContainer({ loading }) {
    ...
    return (
        <div>
            <div className={styles.liked}>Добавлено в избранное: <span>{likedList.length}</span> {likedHotelsText}

            </div>
            
            <div className={styles.itemsContainer} ref={rootRef}>
                {[...hotels].splice(0, shownHotels).map(hotel => 
                    <ElementHotel {...hotel} key={hotel.hotelId} days={days} checkIn={checkIn.split('-')} likedList={likedList}/>)
                }
                {(hasNextPage) && (
